...
async function pdfCotizacion(idOrden) {
  const orden = await OrdenModel.obtener(idOrden);
...
async function pdfResultados(idOrden) {
  const orden = await OrdenModel.obtener(idOrden);
  ...
  if (orden.resultado_heces.length) {
    const [cats] = await db.execute('SELECT * FROM categoria_heces WHERE estado = 1 ORDER BY orden');
    const [params] = await db.execute('SELECT * FROM parametros_heces');
  ...
  if (orden.resultado_orina.length) {
    const [cats] = await db.execute('SELECT * FROM categoria_orinas WHERE estado = 1 ORDER BY orden');
    const [params] = await db.execute('SELECT * FROM parametros_orinas');
...
module.exports = { pdfCotizacion, pdfResultados };
